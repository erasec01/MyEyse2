...
{
    public float storeCriticVal = 10;
    //da cancellare
    public static List<Float> accValue = new ArrayList<>();
    public List<Float> f = new ArrayList<Float>();
     ...
     * This value is chosen empirically using a testbed environment.
     */
     ...
     * Defines
     */
    private final static float RANGE_INTERVAL_STOP = 1f;
     ...
     */
    private DirectOrientation mClockWiseRotation;
    ...
    //Reference to the class that is used to register / de-register the sensors and whit a frequency
    //of.. a new value is given
    ...
    //Constructor
    public ManageRotation(Context context)
    {
        mContext = context;

        //Set the default value to the variable previously described
        mIsRotationChange = false;
        mClockWiseRotation = DirectOrientation.UNDEFINED_ROTATION;
    ...
    {
        return (mClockWiseRotation != DirectOrientation.UNDEFINED_ROTATION);
    }
    ...
    public boolean UserInsideNearInitialPosition()
    {
        ...
        if (userWithinInitialPositionRange == false)
        {
            if (mClockWiseRotation == DirectOrientation.CLOCKWISE_ROTATION)
            {
                if ((current_position >= MAX_POSITION - 1) && (half_tour_complete == true))
                {
            ...
            {
                if ((current_position <= RANGE_INTERVAL_STOP) && (half_tour_complete == true))
                {
    ...
    public boolean StopUser()
    {
        ...
        if (userWithinInitialPositionRange == true)
        {
            if (mClockWiseRotation == DirectOrientation.CLOCKWISE_ROTATION)
            {
    ...
    {
        return convertPosToDegree(mPositions.mCurrentPosition);
    }
     ...
     * @return
     */
    private InfoBack convertPosToDegree(float pos)
    {
        //We used the proportion PI:180 = pos:x
        float degree = ((pos * 180) / (float) Math.PI);

        if (degree > 180)
        {
            int degreeInt = ((int) (360 - degree));
            int offset = (degreeInt % 10);
            updateSet(DirectOrientation.CLOCKWISE_ROTATION);
            return new InfoBack(degreeInt - offset, "clockwise");
        }
        else
        {
            int degreeInt = ((int) (degree));
            int offset = (degreeInt % 10);
            updateSet(DirectOrientation.COUNTERCLOCKWISE_ROTATION);
            return new InfoBack(degreeInt - offset, "counterclockwise");
        }
    }
     ...
     * @param orientation
     */
    private void updateSet(DirectOrientation orientation)
    {
        half_tour_complete = true;
        mClockWiseRotation = orientation;
        mIsRotationChange = false;
        mNumWarningSendToUser = 0;
        mInstantLastWarning = 0;
        mMaximumAngularSpeedExceeded = false;
    ...
    private void updateInfoOrientation(float[] gyValue)
    {
        ...
        if (!mIsRotationChange)
        {
             ...
             */
            if (mClockWiseRotation != DirectOrientation.UNDEFINED_ROTATION)
            {
                ...
                if (half_tour_complete == false)
                {
                    ...
                    if ((current_position > POS_HALF_TOUR - 0.5) &&
                            ...
                            (mClockWiseRotation == DirectOrientation.CLOCKWISE_ROTATION))
                        half_tour_complete = true;
                    /**
                     ...
                     */
                            ...
                            (current_position > POS_HALF_TOUR - 0.5) &&
                            (mClockWiseRotation == DirectOrientation.COUNTERCLOCKWISE_ROTATION))
                        half_tour_complete = true;
                }
            }
            else
    ...
    private void checkConsistencyUser(float[] value)
    {
        ...
        else if (((mClockWiseRotation == DirectOrientation.CLOCKWISE_ROTATION) &&
                (value[1] > 0.3f)) ||
                ((mClockWiseRotation == DirectOrientation.COUNTERCLOCKWISE_ROTATION) &&
                        (value[1] < -0.3f)))
        {
    ...
    private void checkIfOrientationDirectionIsEstablished(float[] gyValue)
    {
                ...
                (statPosition <= RANGE_INTERVAL_START))
        {
            if (statPosition >= MAX_POSITION - RANGE_INTERVAL_START)
            {
                if ((currentPosition < 1) && (currentPosition > increment(mPositions.getStartPositionPanoramicMode(),
    ...
    private class ManageSensors implements SensorEventListener
    {
        ...
        public void onSensorChanged(SensorEvent event)
        {
            ...
            //azimuthData = magAccPosition[0];

            //Calculate position from accelerometer magnetometer with raw data low pass filtered
            //mAccDataFiltered = lowPassFilter(event.values.clone(), mAccDataFiltered);
            //mMagDataFiltered = lowPassFilter(event.values.clone(), mMagDataFiltered);
            ...
            if (rotationOk2)
            {
                ...
                else if ((mPositions == null) && (ALGORITHM_FOR_POSITION == AlgorithmForPosition.LOW_PASS))
                {
            ...
            if (sensorType == Sensor.TYPE_GYROSCOPE)
            {
                ...
                //azimuthDataMagAccGyro = fusedPosition[0];
            }
        }
